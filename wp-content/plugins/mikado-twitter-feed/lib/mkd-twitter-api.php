...
class MikadoTwitterApi {
				...
				if((is_array($responseParsed) && count($responseParsed)) &&
				   !empty($responseParsed['oauth_token']) && !empty($responseParsed['oauth_token_secret'])
					...
					if(!empty($responseObj->redirectUrl)) {
						$responseObj->status  = false;
						$responseObj->message = esc_html__('Redirect URL couldn\'t be generated', 'mkd-twitter-feed');
					} else {
						$responseObj->status  = true;
						$responseObj->message = esc_html__('Ok', 'mkd-twitter-feed');
					}
				} else {
					$responseObj->status  = false;
	...
	public function obtainAccessToken() {
		...
		if(!empty($authorizeVerifier) && !empty($authorizeToken)) {
			...
			if(is_wp_error($response)) {
				$responseObj->status  = false;
				$responseObj->message = esc_html__('Internal WP error', 'fleur');
			} else {
				$responseBody = wp_remote_retrieve_body($response);
				parse_str($responseBody, $responseParsed);
				if(is_array($responseParsed) && count($responseParsed)
			...
			$responseObj->status  = false;
			$responseObj->message = esc_html__('Authorize token and it\'s secret were not obtained', 'mkd-twitter-feed');
		}
	...
	public function fetchTweets($userId = '', $count = '', $transient = array()) {
		...
		if(!$this->transientExists($transient)) {
			if($userId && $accessToken) {
				...
				$responseObj->status  = false;
				$responseObj->message = esc_html__('It seems like you haven\'t connected with your Twitter account', 'mkd-twitter-feed');
			}
		} else {
			...
			if(!empty($transientContent)) {
				...
				$responseObj->status  = false;
				$responseObj->message = esc_html__('Couldn\'t retrieve content from database', 'mkd-twitter-feed');
			}
		}
