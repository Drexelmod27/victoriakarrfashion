...
if (!function_exists('mkd_core_add_user_custom_fields')) {
    ...
    function mkd_core_add_user_custom_fields($user_contact)
    {
         ...
         **/
        $user_contact['position'] = esc_html__('Position', 'mkd_core');
        $user_contact['instagram'] = esc_html__('Instagram', 'mkd_core');
        $user_contact['twitter'] = esc_html__('Twitter', 'mkd_core');
        $user_contact['pinterest'] = esc_html__('Pinterest', 'mkd_core');
        $user_contact['tumblr'] = esc_html__('Tumbrl', 'mkd_core');
        $user_contact['facebook'] = esc_html__('Facebook', 'mkd_core');
        $user_contact['google-plus'] = esc_html__('Google Plus', 'mkd_core');
        $user_contact['linkedin'] = esc_html__('Linkedin', 'mkd_core');
...
if ( class_exists( 'WP_Block_Type' ) && defined( 'MIKADO_ROOT' ) ) {
	add_action( 'admin_head', 'fleur_mikado_meta_box_add' );
} else {
	add_action( 'add_meta_boxes', 'fleur_mikado_meta_box_add' );
}

if ( ! function_exists( 'fleur_mikado_create_meta_box_handler' ) ) {
	function fleur_mikado_create_meta_box_handler( $box, $key, $screen ) {
		add_meta_box(
			'mkd-meta-box-' . $key,
			$box->title,
