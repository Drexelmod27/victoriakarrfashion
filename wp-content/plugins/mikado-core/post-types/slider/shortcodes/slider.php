			...
			if(have_posts()) : $postCount = 0;
				while(have_posts()) : the_post();
					...
					if($slide_type == 'video') {
						...
						$html .= '>';
						if($video_overlay_image != "") {
							$html .= '<img src="'.esc_url($video_overlay_image).'" alt="'.esc_attr("Video overlay image", "mkd_core").'" />';
						} else {
							$html .= '<img src="'.esc_url(get_template_directory_uri()).'/assets/css/img/pixel-video.png" alt="'.esc_attr("Video overlay image", "mkd-core").'" />';
						}
						...
						$html .= '<object width="320" height="240" type="application/x-shockwave-flash" data="'.esc_url(get_template_directory_uri()).'/assets/js/flashmediaelement.swf">
													...
													<img src="'.esc_url($video_image).'" width="1920" height="800" title="'.esc_attr__("No video playback capabilities", "mkd_core").'" alt="'.esc_attr__("Video thumb", "mkd_core").'" />
											</object>
									</video>
							</div></div>';
					} else {
					...
					if(($meta_temp_anchor = get_post_meta(get_the_ID(), "mkd_slide_scroll_to_section", true)) !== '') {
						...
						$html_slider_anchor = '<div class="mkd-slide-anchor-holder '.esc_attr($slide_anchor_classes).'"><a '.$slide_anchor_style.' class="mkd-slide-anchor-button mkd-anchor" href="'.esc_url($meta_temp_anchor).'"><i class="icon-mouse"></i><span class="scroll-text">'.esc_html__("Scroll Down", "fleur").'</span></a></div>';
					}
				...
				$html .= esc_html__('Sorry, no slides matched your criteria.', 'mkd_core');
			endif;
			wp_reset_query();
